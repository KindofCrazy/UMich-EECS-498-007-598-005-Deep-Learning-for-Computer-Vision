...
def compute_distances_no_loops(x_train, x_test):
  ...
  # Helped by copilot
  # still not familiar with Vectorization
  # practices are needed

  train = x_train.reshape(num_train, -1)
  test = x_test.reshape(num_test, -1)
  train_square_sum = torch.sum(train ** 2, dim=1).reshape(num_train, 1)
  test_square_sum = torch.sum(test ** 2, dim=1).reshape(1, num_test)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_folds = list(torch.chunk(input=x_train, chunks=num_folds, dim=0))
  y_train_folds = list(torch.chunk(input=y_train, chunks=num_folds, dim=0))
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      train_data = torch.cat([x_train_folds[m] for m in range(num_folds) if m != i], dim=0)
      train_labels = torch.cat([y_train_folds[m] for m in range(num_folds) if m != i], dim=0)
