...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  idx = torch.arange(x.shape[0]).tolist()
  idx.reverse()
  y = x[idx]
...
def count_negative_entries(x):
  ...
  # Replace "pass" statement with your code
  mask = (x < 0)
  num_neg = count_tensor_elements(x[x < 0])
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_neg
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone()
  row_min_values, row_min_idxs = y.min(dim=1)
  row_idx = torch.arange(y.shape[0])
  y[row_idx, row_min_idxs] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  square = torch.sum((x - mu) ** 2, dim=0)
  sigma = torch.sqrt( square / (x.shape[0] - 1))
  y = (x - mu) / sigma
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
